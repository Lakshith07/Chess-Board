...
$(document).ready(function () {
    ...
    $('.gamecell').click(function (e) {
        ...
        } else if (main.variables.selectedpiece != '' && target.name != 'null' && target.id != selectedpiece.id &&
    ...
    $(document).keydown(function (e) {
        if ($('#game').is(':visible')) {
            if (!main.variables.selectedpiece) {
                if (!main.variables.currentHighlightedCell) {
                    main.variables.currentHighlightedCell = main.variables.turn == 'w' ? '1_2' : '1_7';
                    $('#' + main.variables.currentHighlightedCell).addClass('selected');
                } else {
                    let [x, y] = main.variables.currentHighlightedCell.split('_').map(Number);
                    if (e.key == 'ArrowRight' && x < 8) x++;
                    else if (e.key == 'ArrowLeft' && x > 1) x--;
                    else if (e.key == 'ArrowUp' && y < 8) y++;
                    else if (e.key == 'ArrowDown' && y > 1) y--;
                    let newCell = `${x}_${y}`;
                    if ($('#' + newCell).attr('chess') != 'null' && $('#' + newCell).attr('chess').slice(0, 1) == main.variables.turn) {
                        $('.gamecell').removeClass('selected');
                        main.variables.currentHighlightedCell = newCell;
                        $('#' + newCell).addClass('selected');
                    }
                }
                if (e.key == 'Enter') {
                    let cell = main.variables.currentHighlightedCell;
                    if ($('#' + cell).attr('chess') != 'null' && $('#' + cell).attr('chess').slice(0, 1) == main.variables.turn) {
                        main.variables.selectedpiece = cell;
                        main.methods.moveoptions($('#' + cell).attr('chess'));
                        $('.gamecell').removeClass('selected');
                ...
                if (main.variables.highlighted.length > 0) {
                    if (!main.variables.currentHighlightedCell || main.variables.highlighted.indexOf(main.variables.currentHighlightedCell) == -1) {
                        main.variables.currentHighlightedCell = main.variables.highlighted[0];
                        $('.gamecell').removeClass('neonorange_txt');
                        $('#' + main.variables.currentHighlightedCell).addClass('neonorange_txt');
                    } else {
                        let currentIndex = main.variables.highlighted.indexOf(main.variables.currentHighlightedCell);
                        if (e.key == 'ArrowRight' || e.key == 'ArrowUp') {
                            currentIndex = (currentIndex + 1) % main.variables.highlighted.length;
                        } else if (e.key == 'ArrowLeft' || e.key == 'ArrowDown') {
                            currentIndex = (currentIndex - 1 + main.variables.highlighted.length) % main.variables.highlighted.length;
                        }
                        $('.gamecell').removeClass('neonorange_txt');
                        main.variables.currentHighlightedCell = main.variables.highlighted[currentIndex];
                        $('#' + main.variables.currentHighlightedCell).addClass('neonorange_txt');
                    }
                    if (e.key == 'Enter') {
                        let target = {
                            name: $('#' + main.variables.currentHighlightedCell).attr('chess'),
                            id: main.variables.currentHighlightedCell
                        };
                        if (target.name == 'null') {
                            if (main.variables.selectedpiece == '5_1' && target.id == '7_1' &&
                                main.variables.pieces['w_king'].moved == false && main.variables.pieces['w_rook2'].moved == false) {
                                main.variables.pieces['w_king'].position = '7_1';
                                main.variables.pieces['w_king'].moved = true;
                                $('#5_1').html('');
                                ...
                                $('#7_1').attr('chess', 'w_king');
                                main.variables.pieces['w_rook2'].position = '6_1';
                                main.variables.pieces['w_rook2'].moved = true;
                                $('#8_1').html('');
                                $('#8_1').attr('chess', 'null');
                                $('#6_1').html(main.variables.pieces['w_rook2'].img);
                                $('#6_1').attr('chess', 'w_rook2');
                                main.methods.endturn();
                            } else if (main.variables.selectedpiece == '5_8' && target.id == '7_8' &&
                                main.variables.pieces['b_king'].moved == false && main.variables.pieces['b_rook2'].moved == false) {
                                main.variables.pieces['b_king'].position = '7_8';
                                main.variables.pieces['b_king'].moved = true;
                                $('#5_8').html('');
                                ...
                                $('#7_8').attr('chess', 'b_king');
                                main.variables.pieces['b_rook2'].position = '6_8';
                                main.variables.pieces['b_rook2'].moved = true;
                                $('#8_8').html('');
                                $('#8_8').attr('chess', 'null');
                                $('#6_8').html(main.variables.pieces['b_rook2'].img);
                                $('#6_8').attr('chess', 'b_rook2');
                                main.methods.endturn();
                            } else {
                                main.methods.move(target);
                                main.methods.endturn();
                            }
                        } else if (target.name != 'null' && main.variables.selectedpiece != target.id &&
                            main.variables.pieces[main.variables.selectedpiece].type.slice(0, 1) != target.name.slice(0, 1)) {
                            if (main.variables.highlighted.indexOf(target.id) != -1) {
                                main.methods.capture(target);
                                main.methods.endturn();
                            }
                        }
                        $('.gamecell').removeClass('selected neonorange_txt');
                        main.variables.currentHighlightedCell = null;
                    }
                }
                if (e.key == 'Escape') {
                    main.methods.togglehighlight(main.variables.highlighted);
